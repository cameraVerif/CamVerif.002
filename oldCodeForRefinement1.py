...
def computeTriangleInvariantRegions3(currTriangle,currGroupName, currGroupRegionCons, posXp, posYp, posZp,mOriginal):
    ...
    # print(currTriangle,currGroupName, currGroupRegionCons, posXp, posYp, posZp,mOriginal)
    ...
    for l in range(0,3):
        for r in range(0,6):
            # print(l,m)
            vertex_plane_cons[l][r] = symComputeOutcodePlane(r, vertices[currTriangleVertices[l]*3+0] -xp0,
...
# # if(s2.check() ==sat):
    ...
    for currVert in intersectingVertices:
        ...
        isIntersect, vertexPixelValue2,intersectionPoint,mp,mq =planeEdgeIntersectionUpdated(currentIntersectingPlane,currInsideVertCoordinates, currOutsideVertCoordinates,mOriginal,1)
    ...
    for l in itertools.product(*combinedDataToPPL):
        ...
        numberOfIntersectingEdges,intersectingEdgeDataToPPL,posXp,posYp,posZp,mOriginal[xp0],mOriginal[yp0],mOriginal[zp0], outcodeP0,currImageName)
        ...
        # scheck.push()
        scheck2.add(And(xp0 ==mOriginal[xp0], yp0 == mOriginal[yp0], zp0 == mOriginal[zp0]))
    ...
    if fA == 0:
        ...
        foundR, consOfReg,minmaxDepths, centerPointImage = oldInvComputation1.computeInv( posXp, posYp, posZp, currTriangle,mOriginal)
        ...
        scheck3.add(And(xp0 ==mOriginal[xp0], yp0 == mOriginal[yp0], zp0 == mOriginal[zp0]))
        if(scheck3.check() != sat or foundR == 0 ):                
            # sleep(3)
            # print("Exiting.....")
            # print(m)
            # print(posXp, posYp, posZp)
...
def computeAllInvariantRegions(currGroupName, currGroupRegionCons, fromSplitRegion=0):
    s100 = Solver()
    ...
    triangleInvRegions.clear()


    s2 = Solver()
    s2.add(currGroupRegionCons)

    programStartTime2 = datetime.now()

    invRegionCount = 0
    while(s2.check() == sat):

        print(invRegionCount+1)
        m = s2.model()
        ...
        # for i in range(0 , 100): 
            # print("\n\n", i)
            currTriangleInvCons, invFound = computeTriangleInvariantRegions3(i,currGroupName, currGroupRegionCons, posXp, posYp, posZp,m)
            
            if(invFound != 0):
                # print("triangle ", i, " contraints ",currTriangleInvCons)
                triangleInvRegions[i] = currTriangleInvCons
        
                


            # print(i," ==> ",currTriangleInvCons)
            # sleep(1)


        #compute intersection of all invariant regions.

        print("Done")
        print(triangleInvRegions)



        print("time Taken = ", datetime.now() - programStartTime2)

        currImageConstraint = And(True)

        for currCons in triangleInvRegions:
            # print(triangleInvRegions[currCons])
            currImageConstraint = And(currImageConstraint, triangleInvRegions[currCons])

        print(simplify(currImageConstraint))
        print("time Taken = ", datetime.now() - programStartTime2)

        s3 = Solver()
        s3.add(simplify(currImageConstraint))
        print(s3.check())
        print(s3.model())
        print("time Taken = ", datetime.now() - programStartTime2)
        s2.add(Not(currImageConstraint))
        invRegionCount += 1
    

    print("Computed all invaraint regions")
    print("invRegionCount = ", invRegionCount)
...
def mogenerateAndCheckOutputOfSmallRegions(currAbsGroupName, currGroupRegionCons, dnnOutput, fromSplitRegion=0):
    s2 = Solver()
    s2.add(currGroupRegionCons)
    ...
    while(s2.check() == sat):
        ...
        # for i in range(0 , 100): 
            # print("\n\n", i)
            currTriangleInvCons, invFound = computeTriangleInvariantRegions3(i,currAbsGroupName, currGroupRegionCons, posXp, posYp, posZp,m)
        ...
        pythonRenderAnImage2.renderAnImage(posXp,posYp, posZp,"collisionImage2_"+str(999098)+"0")
        net_dnnoutput = getDNNOutput_onnx("images/"+str("collisionImage2_"+str(999098))+"0.ppm",'OGmodel_pb_converted.onnx')

        if (dnnOutput == net_dnnoutput):
            regionConsForChecking.append(currImageConstraint)
            
    
    if len(regionConsForChecking) > 0:
        return 1, regionConsForChecking
    else:
        return 0, []
...
# computeAllInvariantRegions( currAbsGroupName, currAbsGroupRegionCons)