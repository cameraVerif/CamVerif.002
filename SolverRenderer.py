...
def computeTriangleInvariantRegions2(currTriangle,currGroupName, currGroupRegionCons):
    ...
    # s2.add(x0 * scale == ToInt(x0 * scale))
    # print("computing invariant regions for triangle ", currTriangle)
    ...
    # if(s2.check() ==sat):
        ...
        if fA ==1:
            ...
            # print("numberOfIntersectingEdges = ", numberOfIntersectingEdges)
...
def computePixelIntervals(currGroupName, currGroupRegionCons, imgName, fromSplitRegion=0):
    # print(mProj)
    ...
    for i in range(0,imageWidth*imageHeight):
        if finalGlobalIntervalImage.get(i):
            ...
            image[i * 3 + 0] = max(0, min(255, abs(int(finalGlobalIntervalImage[i][0]))))
            image[i * 3 + 1] = max(0, min(255, abs(int(finalGlobalIntervalImage[i][2]))))
            image[i * 3 + 2] = max(0, min(255, abs(int(finalGlobalIntervalImage[i][4]))) ) 

            tempFile.write(str(max(0, min(255, abs(int(finalGlobalIntervalImage[i][0])))))+str("\n"))
            tempFile.write(str(max(0, min(255, abs(int(finalGlobalIntervalImage[i][2])))))+str("\n"))
            tempFile.write(str(max(0, min(255, abs(int(finalGlobalIntervalImage[i][4])))) )+str("\n"))
...
def solver_rendrer():
    ...
    xfraction = Fraction(xPos).limit_denominator()
    xnumerator = xfraction.numerator
    xdenominator = xfraction.denominator

    yfraction = Fraction(yPos).limit_denominator()
    ynumerator = yfraction.numerator
    ydenominator = yfraction.denominator

    zfraction = Fraction(zPos).limit_denominator()
    znumerator = zfraction.numerator
    zdenominator = zfraction.denominator

    XCons = str(xdenominator) + "* xp0 == " +  str(xnumerator)
    YCons = str(ydenominator) + "* yp0 == " +  str(ynumerator)
    ZCons = str(zdenominator) + "* zp0 == " +  str(znumerator)

    updateExp = [XCons, YCons, ZCons]
    ...
    conFile.close()
    createPoly2 = reload(createPoly)
    pd4 = createPoly2.getPoly()

    environment.singleImagePolyh = pd4.minimized_constraints()

    xp0, yp0, zp0 = Reals('xp0 yp0 zp0')
    currRegionCons = And(eval(XCons), eval(YCons), eval(ZCons))
   
    print("Rendering  at : ", xPos, yPos, zPos)
