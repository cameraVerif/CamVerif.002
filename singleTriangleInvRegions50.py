...
def computeTriangleInvariantRegions2(currTriangle,currGroupName, currGroupRegionCons):
    ...
    if(vertices[currTriangleVertices[0]*3+2] > posZp100-1 + depthOfTheInitialCube and 
       vertices[currTriangleVertices[1]*3+2] > posZp100-1 + depthOfTheInitialCube and 
       vertices[currTriangleVertices[2]*3+2] > posZp100-1 + depthOfTheInitialCube ):
        return 0
    ...
    # if(s2.check() ==sat):
        ...
        if(not any(outcodeP0)):
            ...
            for currVert in range(0,3):        
                ...
                # cons1 = ( (((-68.39567*(x -m[xp0]) )/ (z -m[zp0]) )+ 24.5 ) )
                # cons2 =  ( (((68.39567*(y -m[yp0]) )/ (z -m[zp0]) )+ 24.5 )  )
                ...
                # xPixelCons1 = ( (((-68.39567*(x -xp0) )/ (z-zp0) )+ 24.5 )  ) < xpixel+1
                # xPixelCons2 = xpixel <= ( (((-68.39567*(x -xp0) )/ (z-zp0) )+ 24.5 )  )


                # yPixelCons1 = ( (((68.39567*(y -yp0) )/ (z-zp0) )+ 24.5 )  ) < ypixel+1
                # yPixelCons2 = ypixel <= ( (((68.39567*(y -yp0) )/ (z-zp0) )+ 24.5 )  )


                # regionCons = And(xPixelCons1,xPixelCons2,yPixelCons1,yPixelCons2)
                # print(regionCons)
                
                # currListOfConsToAdd.append(regionCons)
                ...
                # currPixelValue = []
                # currPixelValue = [vertexPixelValue[0], vertexPixelValue[1]]
                # if currVert == 0:
                ...
                # currVertexPixelData = [] 
                # currVertColour = environment.vertColours[currTriangleVertices[currVert]*3:currTriangleVertices[currVert]*3+3]
                # tempPixelData = [currTriangleVertices[currVert], currPixelValue[0],currPixelValue[1],currVert,currVertColour]
            ...
            if(not vert0_pos):
                ...
                # 68.3951428680815276799045792507503668934470873440165530959340458671429810179786198093854102574863616004
                # 68.3951428680815361913365392828734570996486352466879036161073778492602945052080424307549865937100741178               
        ...
        set_option(rational_to_decimal=False)
        ###new region computation using z3 only
        # print("CurrListOFCOns = ", currListOfConsToAdd)
        
        # tempRegionCons2 = And(True)
        # for currCons in currListOfConsToAdd:
        #     tempRegionCons2 = And(tempRegionCons2, currCons)
            
        # # my_string = ','.join(currListOfConsToAdd)
        
        # # tempRegionCons2 += my_string+"))"
        
        # print(tempRegionCons2)
        # print(simplify(tempRegionCons2))
        
        
       
        
        # s2.add(Not(simplify(tempRegionCons2)))
        # continue
        
        ########Z3 region computation ends here
        ...
        for currVert in intersectingVertices:
            ...
            # isIntersect = 1
            # tempData, temp = pixelValue(tr_vertex_coordinates[currIntersectingData[0]], tr_vertex_ws[currIntersectingData[0]]) 
            # vertexPixelValue2 = [tempData[0], tempData[1]]
            ...
            # currIntersecingOutvalue = tr_vertex_coordinates[currVert]
            # currIntersecingW = tr_vertex_ws[currVert]
            # currIntersectingPixel, temp = pixelValue(currIntersecingOutvalue, currIntersecingW)
            # print("Intersecting point pixel value = ", currIntersectingPixel, temp)
            
            # singleIntersectingData = [-1, currInsideVertCoordinates,currOutsideVertCoordinates,currentIntersectingPlane,\
            #                                          currIntersectingPixel[0],currIntersectingPixel[1]]   
            # globalIntersectingVertexDataToPPL.append([singleIntersectingData])
            # numberOfIntersectingEdges +=1
        ...
        ####end of region computation for 27/40####          
        ...
        #########################################################
        #########################################################
        ###TODO: change to New implementation of depth computation. ##############
        ########################################################
        #######################################################
...
def computePixelIntervals(currGroupName, currGroupRegionCons, fromSplitRegion=0):
    ...
    print(" number of inv regions : ", numberOfreg)
    
    
    # equalFlag = 1
    # for currPixelsinGlobalInterval in range(0,49*49):
    #     if finalGlobalIntervalImage.get(currPixelsinGlobalInterval):
    #         #  print(currPixelsinGlobalInterval)
    #         if ((finalGlobalIntervalImage[currPixelsinGlobalInterval][0] == finalGlobalIntervalImage[currPixelsinGlobalInterval][1]) and
    #         (finalGlobalIntervalImage[currPixelsinGlobalInterval][2] == finalGlobalIntervalImage[currPixelsinGlobalInterval][3]) and
    #         (finalGlobalIntervalImage[currPixelsinGlobalInterval][4] == finalGlobalIntervalImage[currPixelsinGlobalInterval][5])):
    #             pass
    #         else:
    #             print(currPixelsinGlobalInterval,", ", finalGlobalIntervalImage.get(currPixelsinGlobalInterval))
    #             equalFlag = 0
    #             break
    
    # print("Equla flag = ", equalFlag)
    # sleep(5)
    ...
    # print("deactivating abc")
    # sleep(10)
    # tempString ="/bin/bash & conda init & conda activate alpha-beta-crown"
    # os.system(tempString)
    # print("activated alaphbetacrown")
    # sleep(10)
    ...
    # nnenumFlag = environment.nnenumFlag
    # #if we have only nnenum supported layers and activation functions, we can use the following also, it will be faster. TODO:output to file
    # if(nnenumFlag==1):
    #     os.environ["OPENBLAS_NUM_THREADS"]="1"
    #     os.environ["OMP_NUM_THREADS"]="1"
    #     tempString = "python -m nnenum.nnenum /home/habeeb/project2/7copiesOSMUpdatedForError/OGmodel_pb_converted.onnx /home/habeeb/project2/7copiesOSMUpdatedForError/prop_y0.vnnlb"
    #     os.system(tempString)
    #     tempString = "python -m nnenum.nnenum /home/habeeb/project2/7copiesOSMUpdatedForError/OGmodel_pb_converted.onnx /home/habeeb/project2/7copiesOSMUpdatedForError/prop_y1.vnnlb"
    #     os.system(tempString)
    #     tempString = "python -m nnenum.nnenum /home/habeeb/project2/7copiesOSMUpdatedForError/OGmodel_pb_converted.onnx /home/habeeb/project2/7copiesOSMUpdatedForError/prop_y2.vnnlb"
    #     os.system(tempString)
    ...
    # tempString ="conda init & conda activate abc"
    # os.system(tempString)
    sleep(2)
    
    
    
    # print(y0Out1, y1Out1,y2Out1)
    
    # deepPolyOutputs3 = []
    
    # if(y0Out1 == "sat"):
    #     deepPolyOutputs3.append(0)
    
    # if(y1Out1 == "sat"):
    #     deepPolyOutputs3.append(1)
    
    # if(y2Out1 == "sat"):
    #     deepPolyOutputs3.append(2)
    # print("sleeping")
    
        
    ###single pos check ####
    
    # print("single image check")
    # global testPosZp,testPosYp,testPosXp
    
    # rendererPosFile = open("imagePoses.txt",'w')    
    # rendererPosFile.write(str(1)+"\n");
    # rendererPosFile.write("testImage\n"); 
    # rendererPosFile.write(str(testPosXp)+"\n"\
    #     +str(testPosYp)+"\n"\
    #     +str(testPosZp)+",\n")
    ...
    # tempstring = "./renderImageAtPosClipped6_image1"
    # os.system(tempstring)
  
    
    # print("running dnn")
    # print("images/"+str("testImage0.ppm"))
    # # currImageDnnOutput= getDNNOutput("images/"+str("testImage0.ppm"))
    # # currImageDnnOutput = 1
    # print("running on iisc model")
    # iisc_net_dnnoutput = getDNNOutput_onnx("images/"+str("testImage0.ppm"),'model3_3.onnx')
    # print("running on OG model")
    # og_net_dnnoutput = getDNNOutput_onnx("images/"+str("testImage0.ppm"),'OGmodel_pb_converted.onnx')
    
    # print("running loadmodel og")
    # og_net_dnnoutput2 = getDNNOutput("images/"+str("testImage0.ppm"))
    # # print(currImageDnnOutput, previousDNNOuput)
    
    # print("OGmodel dnn output = ", og_net_dnnoutput)
    # print("OGmodel load dnn output = ", og_net_dnnoutput2)
    # print("Alpha beta ognet Done, outputs = ", deepPolyOutputs)

    
    # print("iisc net dnn output 3_3 = ", iisc_net_dnnoutput)
    # print("Alpha beta iiscnet Done, outputs 3_3 = ", deepPolyOutputs3)
    # # print("Expected output = ", previousDNNOuput)
    
    
    # ####single pos check end ###
    
    # # deepPolyOutputs = [0,2,1]
    
    # # print("deeppoly dummy outputs = ", deepPolyOutputs2)
    ...
    # rendererPosFile = open("imagePoses.txt",'w')    
    # rendererPosFile.write(str(1)+"\n");
    # rendererPosFile.write("testImage_"+str(1)+"\n"); 
    # rendererPosFile.write(str(posXp2)+"\n"\
    #     +str(posYp2)+"\n"\
    #     +str(posZp2)+",\n")
    ...
    # print("running dnn")
    # print("images/"+str("testImage_"+str(1))+"0.ppm")
    # # currImageDnnOutput= getDNNOutput("images/"+str("collisionImage2_"+str(i))+"0.ppm")
    # # # currImageDnnOutput = 1
    # print("running on second model")
    # iisc_net_dnnoutput = getDNNOutput_onnx("images/"+str("testImage_"+str(1))+"0.ppm",'OGmodel_pb_converted.onnx')
    ...
    # iisc_net_dnnoutput = getDNNOutput_onnx("images/testImage1.ppm",'OGmodel_pb_converted.onnx')
    ...
    # if int(fromSplitRegion) == 0:
    #     if len(currGroupName) < 12:
    #         for o in range(0,len(deepPolyOutputs)):
    #             # if(deepPolyOutputs[o] == og_net_dnnoutput):
    #             #     continue
    #             environment.absStack.append(currGroupName+str(deepPolyOutputs[o]))
    ...
    #             # if(deepPolyOutputs[o] == og_net_dnnoutput):
    #             #     continue
    #             environment.absStack.append(currGroupName+str(deepPolyOutputs[len(deepPolyOutputs)-1-o]))
                
    #             currentNodeParentName = currGroupName[:currGroupName.rfind("_")]
    #             currentNode = currGroupName+str(deepPolyOutputs[len(deepPolyOutputs)-1-o])
    #             currentNodeParent = anytree.find(environment.A, filter_=lambda node: node.name==currentNodeParentName)
    #             currentNode = anytree.Node(currentNode, parent=currentNodeParent)
