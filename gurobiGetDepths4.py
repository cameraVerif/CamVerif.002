...
def getDepthIntervals3(currImageSetConString,dataToComputeDepth,
    ...
    elif currFrustumPlane == 5:
        # print("near plane")
        #near plane
        frustumPlaneCons.append("u*"+str(fTL[0])+"+v*"+str(fTR[0])+"+w*"+str(fBL[0])+"+g*"+str(fBR[0])+" == xf0")
        frustumPlaneCons.append("u*"+str(fTL[1])+"+v*"+str(fTR[1])+"+w*"+str(fBL[1])+"+g*"+str(fBR[1])+" == yf0")
        frustumPlaneCons.append("u*"+str(fTL[2])+"+v*"+str(fTR[2])+"+w*"+str(fBL[2])+"+g*"+str(fBR[2])+" == zf0")
    ...
    currInsideVertex = insideVertex
    currOutsideVertex = outsideVertex
    # print(">>>>>>>>>>>>>>>>TOREMOVE")
    # currOutsideVertex = [0,1,200]   
    
    lineConsList.append("p*"+str(currInsideVertex[0])+"+q*"+str(currOutsideVertex[0])+" == xl")
    lineConsList.append("p*"+str(currInsideVertex[1])+"+q*"+str(currOutsideVertex[1])+" == yl")
    lineConsList.append("p*"+str(currInsideVertex[2])+"+q*"+str(currOutsideVertex[2])+" == zl")
    ...
    # pixelConsList = ["-68.39567*(xl-xp0) <= ("+ str(xpixel+1)+"-24.5)*(zl-zp0)", "68.39567*(yl-yp0) <= ("+ str(ypixel+1)+"-24.5)*(zl-zp0)",
    #                  "-68.39567*(xl-xp0) >= ("+ str(xpixel)+"-24.5)*(zl-zp0)", "68.39567*(yl-yp0) >= ("+ str(ypixel)+"-24.5)*(zl-zp0)"]
    ...
    if m.Status <= 2:
        ...
        # print(-68.39567*((xl.X-xp0.X)/(zl.X-zp0.X))+24.5)
        # print(68.39567*((yl.X-yp0.X)/(zl.X-zp0.X))+24.5)
...
def getDepthIntervals4(currImageSetConString,dataToComputeDepth,
    ...
    # obj =  ((xp0 - xl)**2 + (yp0 - yl)**2 + (zp0 - zl)**2) #####original distance
    obj =  ( (zp0 - zl)**2)
    m.setObjective(obj, GRB.MINIMIZE)
    # m.setObjective(obj, GRB.MAXIMIZE)
    m.optimize()
