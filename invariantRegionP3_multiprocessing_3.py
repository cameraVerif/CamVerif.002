...
def removeEdges(outsideVertex,edges):
...
def getInvariantRegion(currTriangle, posXp, posYp, posZp, m,currGroupName):
    ...
    if(not any(outcodeP0)):            
        ...
        for currPlane in [5,0,1,2,3]:
            ...
            while(num_of_outside_vertices > 2):
                ...
                for currOutsideVert in tr_outside_vertex_set:               
                    ...
                    if(tr_outside_vertex_set.count(edge1) and tr_outside_vertex_set.count(edge2)):
                        ...
                        removeEdges(currOutsideVert,edges)
            ...
            if num_of_outside_vertices == 1:
                ...
                removeEdges(outsideVertex,edges)
                ...
                removeEdges(outsideVertex1, edges)
                ...
                removeEdges(outsideVertex2, edges)      
    ...
    currImageName = currGroupName+"_"+str(1)
...
def computeInvRegions(currGroupName, currGroupRegionCons, dnnOutput, uniqueId,  fromSplitRegion=0):
    ...
    while(s2.check() ==sat):
        ...
        for currInv in triaglesInvRegions.values():
            ...
            currInvConsStringList.append(str(currInv))
        ...
        # print("running dnn")
        # print("images/"+str(uniqueId)+"RefineImage2_0.ppm")
        ...
        if iisc_net_dnnoutput == dnnOutput:
            ...
            foundARegion = 1
            regionsToReturnStringList.append(currInvConsStringList)
        
        print("dnnOutput: ", uniqueId," : ",dnnOutput)
    ...
    # print("regionsToReturn: ", regionsToReturn)
    # print("uniqueId: ", uniqueId)
    # print(type(uniqueId))
    # print(type(foundARegion))
    # # print(type(regionsToReturn))
    # print(len(regionsToReturnStringList))
    # # print("\n\nregionsToReturnStringList ",uniqueId, regionsToReturnStringList)
    # print("putting data to the queue")
    # queue.put([foundARegion, regionsToReturnStringList,  uniqueId])
    # return foundARegion,  uniqueId
    # print(queue.isempty())
    
    #write details to a file with uniqueid as name
    fileName = str("tempFiles/")+str(uniqueId)+".txt"
    uniqueFile = open(fileName, 'w')
    uniqueFile.write(str(uniqueId)+"\n")
    uniqueFile.write(str(numberOfInvRegions)+"\n")
    uniqueFile.write(str(foundARegion)+"\n"+str(len(regionsToReturnStringList))+"\n")
    uniqueFile.write(str(currGroupName)+"\n")
    uniqueFile.write(str(regionsToReturnStringList))
    uniqueFile.close()
    ...
    # Optionally, put a sentinel value to indicate that the process is done
...
#     refinementStartTime = datetime.now()
#     print("Refinement Started at: ", refinementStartTime)
#     currGroupName ="A_"
...
#     # queue = Queue()  # Queue to collect results
#     processes = []  # List to keep track of processes

#     # Explicitly use dill as the serialization method
#     # import dill
#     # Process._Popen = dill.Pickler
...
#     num_processes =10
#     total_tasks = 100
#     for k in range(0, total_tasks):
#         # foundARegion, regionsToReturn, uniqueIdReturn = computeInvRegions(currGroupName, currGroupRegionCons, k, 1, queue)
#         # print("foundARegion: ", foundARegion)
#         # print("regionsToReturn: ", regionsToReturn)
#         # print("uniqueIdReturn: ", uniqueIdReturn)
#         # print("\n\n\n")
#         # currGroupName, currGroupRegionCons, dnnOutput, uniqueId, queue, fromSplitRegion=0
        
#         uniqueId = k
#         uniqueIdList.append(k)
#         print("\n\n>>>>>starting new child process ", uniqueId)
#         process = Process(target=computeInvRegions, args=(currGroupName, currGroupRegionCons, 1, uniqueId))
...
#     print("time taken to refine: ", datetime.now()-refinementStartTime)
...
#     # while True:
#     #     result = queue.get()  # Block until something is retrieved from the queue
#     #     if result is None:  # Check for the sentinel value
#     #         break
...
#     print("time taken to finish refinment: ", datetime.now()-refinementStartTime)
